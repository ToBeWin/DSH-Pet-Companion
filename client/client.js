window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      greetingOne: '嗨，我在这里陪你。', greetingTwo: '需要帮忙时，叫我一声吧！', greetingThree: '今天也一起完成一件小事。',
    };
    const en = {
      title: 'Pet Companion',
      ...
      greetingOne: 'Hi, I’m here with you.', greetingTwo: 'Need a hand? Just say the word!', greetingThree: 'Let’s finish one small thing together.',
    };
    ...
    function installStyles() {
      ...
      style.textContent = `
        ...
        .tobewin-pet-dock { position: relative; display: grid; width: var(--pet-size); height: var(--pet-size); place-items: end center; pointer-events: auto; isolation: isolate; cursor: pointer; }
        ...
        .tobewin-pet-greeting { position: absolute; z-index: 4; bottom: calc(100% - 14px); width: max-content; max-width: min(230px, calc(100vw - 48px)); padding: 8px 11px; border: 1px solid var(--dsw-alias-border-l1); border-radius: 12px; color: var(--dsw-alias-label-primary); background: var(--dsw-alias-bg-overlay); box-shadow: 0 10px 25px rgba(6, 13, 27, .18); font-size: 12px; line-height: 18px; opacity: 0; pointer-events: none; transform: translateY(6px) scale(.98); transition: opacity .18s ease, transform .18s ease; }
        .tobewin-pet-layer[data-side="right"] .tobewin-pet-greeting { right: 0; } .tobewin-pet-layer[data-side="left"] .tobewin-pet-greeting { left: 0; }
        .tobewin-pet-greeting[data-visible="true"] { opacity: 1; transform: translateY(0) scale(1); }
        ...
        .tobewin-pet-section-label { color: var(--dsw-alias-label-primary); font-size: 13px; font-weight: 600; line-height: 19px; } .tobewin-pet-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; width: 100%; min-width: 0; }
        .tobewin-pet-card { display: grid; grid-template-columns: 72px minmax(0, 1fr); width: 100%; min-width: 0; min-height: 88px; max-height: 88px; gap: 10px; align-items: center; overflow: hidden; border: 1px solid var(--dsw-alias-border-l1); border-radius: 10px; padding: 8px; color: var(--dsw-alias-label-primary); background: var(--dsw-alias-bg-layer-1); contain: layout paint; text-align: left; cursor: pointer; transition: border-color .16s ease, box-shadow .16s ease, transform .16s ease; } .tobewin-pet-card:hover, .tobewin-pet-card[aria-pressed="true"] { border-color: var(--dsw-alias-brand-primary); box-shadow: 0 0 0 2px color-mix(in srgb, var(--dsw-alias-brand-primary) 18%, transparent); } .tobewin-pet-card:hover { transform: translateY(-1px); } .tobewin-pet-card:focus-visible { outline: 2px solid var(--dsw-alias-brand-primary); outline-offset: 2px; }
        .tobewin-pet-card-art { display: grid; width: 72px; height: 72px; min-width: 72px; min-height: 72px; place-items: end center; overflow: hidden; border-radius: 8px; background: radial-gradient(circle at 50% 28%, color-mix(in srgb, var(--dsw-alias-brand-primary) 25%, transparent), transparent 69%), var(--dsw-alias-state-business-secondary); } .tobewin-pet-card-art img { display: block; width: 72px; height: 72px; max-width: 72px; max-height: 72px; object-fit: contain; transform: translateY(5px); filter: drop-shadow(0 6px 7px rgba(7, 12, 24, .20)); } .tobewin-pet-card-name { display: block; overflow: hidden; font-size: 12px; font-weight: 600; line-height: 17px; text-overflow: ellipsis; white-space: nowrap; } .tobewin-pet-card-detail { display: -webkit-box; overflow: hidden; margin-top: 2px; color: var(--dsw-alias-label-secondary); font-size: 11px; line-height: 15px; -webkit-box-orient: vertical; -webkit-line-clamp: 2; } @media (max-width: 680px) { .tobewin-pet-grid { grid-template-columns: minmax(0, 1fr); } }
      `;
      document.head.appendChild(style);
    }

    function createPetLayer(store, translate) {
      let node; let hideGreeting;
      ...
      function render(settings) {
        ...
        root.innerHTML = `<div class="tobewin-pet-dock" role="group" aria-label="${translate('petLabel')}"><button class="tobewin-pet-quick-hide" type="button" aria-label="${translate('quickHide')}" title="${translate('quickHide')}">×</button><div class="tobewin-pet-greeting" aria-live="polite" data-visible="false"></div><div class="tobewin-pet-aura" aria-hidden="true"></div><img class="tobewin-pet-sprite" src="${petArt(settings.pet)}" alt="" draggable="false"></div>`;
        root.querySelector('button')?.addEventListener('click', () => store.set({ enabled: false }));
        const dock = root.querySelector('.tobewin-pet-dock'); const greeting = root.querySelector('.tobewin-pet-greeting');
        const greet = () => { if (!greeting) return; greeting.textContent = translate(['greetingOne', 'greetingTwo', 'greetingThree'][Math.floor(Math.random() * 3)]); greeting.dataset.visible = 'true'; window.clearTimeout(hideGreeting); hideGreeting = window.setTimeout(() => { if (greeting) greeting.dataset.visible = 'false'; }, 3200); };
        dock?.addEventListener('mouseenter', greet); dock?.addEventListener('click', (event) => { if (!event.target.closest('.tobewin-pet-quick-hide')) greet(); });
      }
      return { render, dispose: () => { window.clearTimeout(hideGreeting); node?.remove(); node = undefined; } };
    }
